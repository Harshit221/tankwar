...
import 'package:tankwar/components/explosion.dart';
...
class FireBall extends Ball {
  ...
  Rect rect;
  double radius;
  static double powerFactor = 0.8;
  FireDetails fireDetails =
      FireDetails(angle: 0.0, power: 0.0); //firePosition: Offset.zero,
  double time;
  bool exploded = false;
  Explosion explosion;
...
//  time = t;
//    powerFactor = 0.8;
    double dx = initialPosition.dx +
        powerFactor * fireDetails.power * cos(fireDetails.angle) * time +
        0.04 * gameController.wind.windPower * time * time;
    double dy = initialPosition.dy -
        powerFactor * fireDetails.power * sin(fireDetails.angle) * time +
        0.5 * gameController.wind.gravity * time * time;
    if (dy >= gameController.mountain.points[dx.toInt()].dy) {
      time -= 0.1;
      dx = initialPosition.dx +
          powerFactor * fireDetails.power * cos(fireDetails.angle) * time +
          0.04 * gameController.wind.windPower * time * time;
      dy = initialPosition.dy -
          powerFactor * fireDetails.power * sin(fireDetails.angle) * time +
          0.5 * gameController.wind.gravity * time * time;
      setExplode();
    }
